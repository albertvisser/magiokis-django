"""script to convert old format stories xml file to new format (unused, I think)
"""
...
for x in verhalen.findall("verhaal"):
    ...
    z2 = et.SubElement(y, "field", to="vertel.verteller", name="schrijver",
                       rel="ManyToOneRel")
    z2.text = "1"
    ...
    if t.upper() != verhaal_titel.upper():
        print("\n  ".join(("titel in verhaal ongelijk aan titel in index:",
                           t.encode('iso-8859-1').join(('"', '"')),
                           verhaal_titel.encode('iso-8859-1').join(('"', '"')))))

    for y in verhaal.findall('hoofdstuk'):
        hoofdstuk_id += 1
        z = et.SubElement(root, "object", pk=str(hoofdstuk_id), model="vertel.hoofdstuk")
        z1 = et.SubElement(z, 'field', to="vertel.verhaal", name="verhaal",
                           rel="ManyToOneRel")
        z1.text = verhaal_id
        ...
        z3.text = '\n'.join([q.text if q.text is not None else ''
                             for q in y.findall('alinea')])
